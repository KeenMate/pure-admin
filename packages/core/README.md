...
- **A shared, themeable icon contract — 13 `--pa-icon-*` glyphs tracing to pure-css's `--base-icon-*`.** `--pa-icon-x` now derives from `--base-icon-close`, and core adds `chevron`, `caret`/`-down`/`-up`, `clear`, `remove`, `expand`, `collapse`, `add`, `edit`, `delete`, `search` (Lucide defaults) with matching `.pa-icon--*` modifiers. One `--base-icon-*` override re-skins core components, the shell, and the web components together; `delete` is a trash bin, not an ✕.
- **Hover/active states split off the recessed surface.** Secondary-button and list-item hovers now read the dedicated hover surface (`--pa-hover-bg` ← `--base-hover-bg`) instead of the recessed subtle surface, which read as *raised* in several dark themes. Light mode is unchanged; recessed surfaces (code, tab tracks, tiles, keycaps) keep the subtle surface.
- **Readable form addons and outline buttons in every theme.** Two long-standing dark/low-contrast spots are fixed. Input-group prepend/append labels defaulted to the muted `--base-text-color-2`, which collided with the addon fill in dark themes (invisible in cafeindustrial, faint in nato); they now track `--base-text-color-1`. And outline buttons (`.pa-btn--outline-*`, `--outline-color-N`) used the vivid role/palette *fill* as the label colour — only ~2:1 on white for warning/success/info/color-1 — so the label now mixes 50% fill with `--base-text-color-1` via `color-mix`, which adapts per mode (dark text in light mode, light in dark) for a worst case of ~5:1 across all 16 themes while the border keeps the vivid brand hue.
