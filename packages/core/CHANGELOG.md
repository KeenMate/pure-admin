...
- **Stat square — compact `pa-stat--square-sm` half-height variant.** Stacks with `.pa-stat--square` + a colour modifier and roughly halves the tile: `min-height` 6.4rem (vs 12.8rem), `max-height` 8rem, `min-width` 6.4rem (vs the base 12.8rem — the larger floor overflowed narrow 1/3 KPI columns and the next tile painted over the previous tile's label), `$spacing-md` padding (vs `$spacing-lg`), and smaller cqi-driven number/symbol clamps (number `clamp(2.4rem, 16cqi, 4rem)`, symbol `clamp(1.2rem, 8cqi, 2rem)`). For dense full-width stat strips where the full square reads as an oversized banner. Driven by new `$stat-square-sm-*` variables.
