...
- **`.pa-splitter` — resizable two-pane container with optional collapse-to-rail.** Vanilla JS + SCSS, no dependencies. Lives in `core-components/_splitter.scss`, `src/js/splitter.js`, snippet at `snippets/splitter.html`, demo at `/components/splitter`.
    ...
    - **Demo** has 6 live examples: horizontal split (sidebar + content), vertical split (editor + console), spaced cards (gap + custom gutter), minimize-to-rail-start (file explorer + editor with full BEM card markup `pa-card__title` / `__title-icon` / `__title-text` / `__actions` + interactive `pa-splitter--minimize-mirror` toggle), minimize-to-rail-end (editor + inspector — right-edge rail pattern), persistence reset button. Plus markup / data-attrs / keyboard / JS API reference cards.
