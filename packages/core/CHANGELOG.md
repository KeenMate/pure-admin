...
- **Masked icon primitive (`.pa-icon` / `.pa-icon--x`) — provider-agnostic icons; the profile close/remove X is now a real icon.** Close/remove affordances used inconsistent **text glyphs** (`✕` U+2715 / `×` U+00D7) that render differently across platforms and fonts. New `.pa-icon` primitive draws an SVG in `currentColor` via `mask-image` — so it inherits the button's colour and scales with `--pa-icon-size` (default `1em`) — with each glyph a single `--pa-icon-*` swap point, keeping the framework **icon-set-agnostic** (override `--pa-icon-x` to swap providers; ships the Lucide "x" as a data-URI mask by default, no icon library or asset needed). The profile panel's **close button** and **favourite-remove** buttons now use `<span class="pa-icon pa-icon--x">`. (The remaining close/remove glyphs — modals, alerts, toasts, badges, filter/search clears, file-selector, notifications — are still text glyphs; converting them onto `.pa-icon` is a tracked follow-up.) Themes must be rebuilt.
