...
- **`pureAdmin.config` — a shared UI-behavior baseline, and `mobileBreakpoint` single-sourced from SCSS.** Introduces one overridable object on the runtime namespace for the framework's binding-agnostic defaults, so components stop hardcoding constants and every wrapper (svelte-pure-admin, keen-pure-admin's Phoenix LiveView hooks, …) inherits the same baseline instead of drifting. First key: **`mobileBreakpoint`** — the `768` that was previously written as a literal in ~8 JS spots (`sidebar-resize.js`'s `MOBILE_MAX`, the demo's `window.innerWidth <= 768` checks) *and* as `$mobile-breakpoint` in SCSS, with no single source. It's now emitted once as a `--pa-mobile-breakpoint` CSS variable (from `$mobile-breakpoint`, in both `main.css` and every theme) and **read from there by JS** — the same idiom `sidebar-resize.js` already uses to read its drag bounds from CSS vars — so the CSS media query and the JS guard can't disagree. Consumers override by setting `window.pureAdmin.config.mobileBreakpoint = 900` before init (honoured, not clobbered by the default). App-domain config (permissions, currentUser, date/time formats, button presets) is deliberately **out of scope** for core and belongs in the wrappers; the full key plan + core-vs-wrapper split is in `docs/config-shared-ui-baseline.md`. Themes must be rebuilt to emit the CSS var.
