...
| `tables.html` | 2026-04-24 | `core-components/_tables.scss` (covers `.pa-table`, `.pa-table-container`, `.pa-table-card`), `core-components/_pagers.scss` (covers `.pa-pager`, `.pa-load-more`) | _(this commit)_ |
...
- **Pager/load-more definitions are duplicated** between `_tables.scss` (lines 475-608) and `_pagers.scss` (full file). The two blocks are near-identical — `_pagers.scss` loads last in `_core.scss` so its definitions win. One subtle difference: `_tables.scss` uses `var(--pa-accent)` for the load-more spinner top-border, while `_pagers.scss` uses the raw SCSS `$accent-color`. The duplicate block should be deleted from `_tables.scss`; framework scope, not snippet.
