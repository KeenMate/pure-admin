...
| `icon.html` (**new**) | 2026-08-24 | `core-components/_icons.scss` — documented the mask-based icon primitive: `.pa-icon` + `.pa-icon--x`, the `--pa-icon-size` / `--pa-icon-src` / `--pa-icon-x` tokens, glyph-swap (provider-agnostic mask URL override), the `pa-icon-mask()` pseudo-element mixin, and a11y (aria-hidden glyph + labelled parent). Closed the last `todo` gap. |
