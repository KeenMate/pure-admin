...
- **Popconfirm is physical-only; toasts are logical.** `_popconfirm.scss` uses physical class names (`--left`/`--right`) *and* physical CSS properties throughout (`margin-left`, `left:`, `border-right: none`, etc.) with zero `[dir="rtl"]` handling. `_toasts.scss` uses logical names (`--top-start`/`--top-end`) and `inset-inline-*` + explicit RTL transform overrides. Consequence: in an RTL app, popconfirms don't mirror — an arrow authored to point from the right will still point from the right in RTL, which is backwards for the reading direction. Fixing this is a framework change (rename classes, swap to `inset-inline-*`, invert the arrow side in RTL); snippet is accurate to what SCSS currently does.
